...
class Analisa_upstream extends BaseController
{
    ...
    public function get_datatable_data_upstream()
    {
        ...
        if ($this->request->isAJAX()) {
            ...
            $query      = $db->query("SELECT upstream_hd.*, type_report.name as tipe_pelaporan,
    ...
    public function export_data_upstream()
    {
        ...
        $fileName = 'Data Laporan Upstream '.$site['name'].' '.Date("M Y");
        $sheet->setCellValue('B1', 'Nama Titik Penaatan');
        $sheet->setCellValue('C1', ': '.$site['name']);
        $sheet->setCellValue('B2', "Tgl Pelaporan");
    ...
    public function update_upstream()
    {
        $result           = array();
        $result['status'] = 'danger';
        $result['reason'] = "Data Tidak Boleh Kosong!";
        ...
        $siteWWTPID    = $this->request->getPost('site');
        $upstream_id   = $this->request->getPost('upstream_id');
        $header = [
            'upstream_id'           => $upstream_id,
            'siteWWTPID'            => $siteWWTPID,
            'tgl_pelaporan'         => convertDateSQL(),
        ...
        $builder = $db->table('upstream_hd');
        $builder->set($header);
        $builder->where('upstream_id', $upstream_id);
        if(!$builder->update()){
            $status = false;
            $bagian = " Header ";
        }
        ...
        foreach ($this->request->getPost('parameter') as $key => $value) {
            ...
            $query = $db->query("UPDATE upstream_dt set nilai = '".$detail['nilai']."' WHERE parameter = '".$detail['parameter']."' and upstream_id = '".$detail['upstream_id']."'");
            if(!$query){
                $status = false;
                $bagian = " Detail ";
            }
