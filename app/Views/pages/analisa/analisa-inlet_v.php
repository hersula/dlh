             ...
             <div class="col-lg-12">
                 <div class="card h-100">
                     ...
                     <div class="card-header">
                         <h3 class="mb-0">Data Analisa Inlet</h3>
                         <p class="text-sm mb-0">
                             Halaman ini untuk melihat analisa inlet dari industri yang terdaftar dan tervalidasi.
                         </p>
                     ...
                     <div class="d-flex flex-row-reverse bd-highlight">
                         <div class="p-2 pr-4 bd-highlight">
                             <button id="tambah-laporan" class="btn btn-primary">Tambah Laporan Inlet</button>
 ...
 <!-- Modal -->
 <div class="modal fade" id="modalInlet" tabindex="-1" role="dialog" aria-labelledby="modalInletLabel" aria-hidden="true">
     <div class="modal-dialog modal-lg" role="document">
         <div class="modal-content">
             <div class="modal-header">
                 <h5 class="modal-title" id="modalInletLabel">Hasil Analisa Pelaporan INLET</h5>
 ...
 <!-- MODAL TAMBAH LAPORAN Inlet -->
 <div class="modal fade" id="modalTambahInlet" tabindex="-1" role="dialog" aria-labelledby="modalTambahInletLabel" aria-hidden="true">
     <div class="modal-dialog modal-lg" role="document">
         <div class="modal-content">
             <div class="modal-header">
                 <h5 class="modal-title" id="modalTambahInletLabel">Tambah Laporan Inlet</h5>
                 <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                     <span aria-hidden="true">&times;</span>
                 </button>
             </div>
 ...
 <script>
     ...
     function myDataTables(site = "", cols = "") {
         ...
         let table =
             $("#datatable").DataTable({
                 ...
                 "ajax": {
                     url: "<?= base_url('get_datatable_data_inlet') ?>",
                     type: "POST",
                     data: function(d) {
                         //  d._token = "{{csrf_token()}}"
                         d.site = site
     ...
     function get_columns(site) {
         ...
         $.ajax({
             url: "<?= base_url("Analisa_inlet/get_columns") ?>",
             type: "POST",
             dataType: "json",
             data: {
                 "siteWWTPID": site,
             ...
             if (siteWWTPID != '') {
                 ...
                 swalWithBootstrapButtons.fire({
                     ...
                     if (result.isConfirmed) {
                         ...
                         window.location.href = "<?= base_url("export_data_inlet"); ?>/" + site;
                         swalWithBootstrapButtons.fire(
                             'Excel',
     ...
     $(function() {
         ...
         // get titik penataan
         function get_parameter_inlet(siteWWTPID) {
             let html = "";
             $.ajax({
                 url: "<?= base_url("Analisa_inlet/get_parameter_inlet") ?>",
                 type: "POST",
                 dataType: "json",
                 data: {
         ...
         function creat_parameter(parameter, value = "", id = "") {
             let html = `
                        ...
                        <input type="hidden" name="inlet_dt_id[]" value="${id}" required >
                        </td>
                        </tr>
                        `;
         ...
         $("#tambah-laporan").click(function() {
             ...
             if (siteWWTPID != '') {
                 $("#form-laporan").attr('action', "<?= base_url("add_data_inlet"); ?>");
                 $("#btn-action").text("Submit").show();
                 $('#content_form input[type=text]').each(function() {
                     $(this).val("")
                 });

                 $("#modalTambahInlet").modal("show");
             } else {
                 notify("Pilih Titik Penaatan Terlebih dahulu!", 'danger', 'top', 'center');
             }
         });
         ...
         $(document).on("click", ".btn-edit", function() {
             ...
             $("#form-laporan").attr('action', "<?= base_url("edit_data_inlet"); ?>");
             $("#content_form").empty();

             let content_form = $("#content_form");
             let selected = $(this).closest("tr");
             let siteWWTPID = $(this).data("id");
             let inlet_id = $(this).data("inlet_id");
             let site_name = $(this).closest("tr").find("td:eq(1)").text();
             let tgl = $(this).closest("tr").find("td:eq(2)").text();
             let quartal = $(this).closest("tr").find("td:eq(1)").text();
             let nama_lab = $(this).data("nama_lab");
             ...
             let html;
             html = create_form_pelaporan(tgl, quartal, nama_lab, nomor_akreditasi_lab, nomor_sampling, jenis_sampling, tgl_start_sampling, tgl_end_sampling, tgl_pengambilan, tgl_diterima, titik_kordinat, inlet_id);
             $("#datatable thead tr:eq(0)").find("th.param").each(function() {
                 html += creat_parameter($(this).text(), selected.find("td:eq(" + $(this).index() + ")").text());
             });
             html += `</table> <div><input type="hidden" name="inlet_id" value="${inlet_id}"/></div>`;
         ...
         $("select[name=site]").change(function() {
             ...
             get_parameter_inlet(siteWWTPID)
