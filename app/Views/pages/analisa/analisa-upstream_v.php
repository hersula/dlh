             ...
             <div class="col-lg-12">
                 <div class="card h-100">
                     ...
                     <div class="card-header">
                         <h3 class="mb-0">Data Analisa Upstream</h3>
                         <p class="text-sm mb-0">
                             Halaman ini untuk melihat analisa Upstream dari industri yang terdaftar dan tervalidasi.
                         </p>
                     ...
                     <div class="d-flex flex-row-reverse bd-highlight">
                         <div class="p-2 pr-4 bd-highlight">
                             <button id="tambah-laporan" class="btn btn-primary">Tambah Laporan Upstream</button>
 ...
 <!-- Modal -->
 <div class="modal fade" id="modalUpstream" tabindex="-1" role="dialog" aria-labelledby="modalUpstreamLabel" aria-hidden="true">
     <div class="modal-dialog modal-lg" role="document">
         <div class="modal-content">
             <div class="modal-header">
                 <h5 class="modal-title" id="modalUpstreamLabel">Hasil Analisa Pelaporan Upstream</h5>
             ...
             <div class="modal-body">
                 <div class="row">
                     <table class="table">
                         ...
                         <tr>
                             <th class="w-30">Tgl Sampling</th>
                             <th style="width:10px">:</th>
                             <td>Xxxxx</td>
                         </tr>
                         <tr>
                             <th class="w-30">Tgl Pengambilan</th>
                             <th style="width:10px">:</th>
                             <td>Xxxxx</td>
                         </tr>
                         <tr>
                             <th class="w-30">Tgl Diterima</th>
                             <th style="width:10px">:</th>
                             <td>Xxxxx</td>
                         </tr>
                         <tr>
                         ...
                         <tr>
                             <th class="w-30">Tgl Pelaporan</th>
                             <th style="width:10px">:</th>
                         ...
                         <tr>
                             <th class="w-30">Tgl Divalidasi </th>
                             <th style="width:10px">:</th>
                             <td>2021-08-20 11:20:00</td>
                         </tr>
                     </table>
 ...
 <!-- MODAL TAMBAH LAPORAN Upstream -->
 <div class="modal fade" id="modalTambahUpstream" tabindex="-1" role="dialog" aria-labelledby="modalTambahUpstreamLabel" aria-hidden="true">
     <div class="modal-dialog modal-lg" role="document">
         <div class="modal-content">
             <div class="modal-header">
                 <h5 class="modal-title" id="modalTambahupstreamLabel">Tambah Laporan upstream</h5>
                 <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                     <span aria-hidden="true">&times;</span>
                 </button>
             </div>
 ...
 <script>
     ...
     function myDataTables(site = "", cols = "") {
         ...
         columns[j] = {
             "render": function(data, type, row, meta) {
                 html = `<button class="btn btn-sm btn-info btn-view"  
                        data-upstream_id="${row.upstream_id}" 
         ...
         let table =
             $("#datatable").DataTable({
                 ...
                 "ajax": {
                     url: "<?= base_url('get_datatable_data_upstream') ?>",
                     type: "POST",
                     data: function(d) {
                         //  d._token = "{{csrf_token()}}"
                         d.site = site
     ...
     function get_columns(site) {
         ...
         $.ajax({
             url: "<?= base_url("Analisa_upstream/get_columns") ?>",
             type: "POST",
             dataType: "json",
             data: {
                 "siteWWTPID": site,
             ...
             if (siteWWTPID != '') {
                 ...
                 swalWithBootstrapButtons.fire({
                     ...
                     if (result.isConfirmed) {
                         ...
                         window.location.href = "<?= base_url("export_data_upstream"); ?>/" + site;
                         swalWithBootstrapButtons.fire(
                             'Excel',
     ...
     $(function() {
         ...
         // get titik penataan
         function get_parameter_upstream(siteWWTPID) {
             let html = "";
             $.ajax({
                 url: "<?= base_url("Analisa_upstream/get_parameter_upstream") ?>",
                 type: "POST",
                 dataType: "json",
                 data: {
         ...
         function creat_parameter(parameter, value = "", id = "") {
             let html = `
                        ...
                        <input type="hidden" name="upstream_dt_id[]" value="${id}" required >
                        </td>
                        </tr>
                        `;
         ...
         $("#tambah-laporan").click(function() {
             ...
             if (siteWWTPID != '') {
                 $("#form-laporan").attr('action', "<?= base_url("add_data_upstream"); ?>");
                 $("#btn-action").text("Submit").show();
                 $('#content_form input[type=text]').each(function() {
                     $(this).val("")
                 });
                 $("#modalTambahUpstream").modal("show");
             } else {
                 notify("Pilih Titik Penaatan Terlebih dahulu!", 'danger', 'top', 'center');
             }
         });
         ...
         $(document).on("click", ".btn-edit", function() {
             ...
             $("#form-laporan").attr('action', "<?= base_url("edit_data_upstream"); ?>");
             $("#content_form").empty();

             let content_form = $("#content_form");
             let selected = $(this).closest("tr");
             let siteWWTPID = $(this).data("id");
             let upstream_id = $(this).data("upstream_id");
             let site_name = $(this).closest("tr").find("td:eq(1)").text();
             let tgl = $(this).closest("tr").find("td:eq(2)").text();
             let quartal = $(this).closest("tr").find("td:eq(1)").text();
             let nama_lab = $(this).data("nama_lab");
             ...
             let html;
             html = create_form_pelaporan(tgl, quartal, nama_lab, nomor_akreditasi_lab, nomor_sampling, jenis_sampling, tgl_start_sampling, tgl_end_sampling, tgl_pengambilan, tgl_diterima, titik_kordinat, upstream_id);
             $("#datatable thead tr:eq(0)").find("th.param").each(function() {
                 html += creat_parameter($(this).text(), selected.find("td:eq(" + $(this).index() + ")").text());
             });
             html += `</table> <div><input type="hidden" name="upstream_id" value="${upstream_id}"/></div>`;
         ...
         $("#form-laporan").submit(function(e) {
             ...
             $.ajax({
                 ...
                 success: function(data) {
                     ...
                     $("#modalTambahUpstream").modal("hide");
                 }
             });
         ...
         $("select[name=site]").change(function() {
             ...
             get_parameter_upstream(siteWWTPID)
